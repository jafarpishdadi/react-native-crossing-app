...
class BridgeUtils {
    ...
    static func parseEasingOption(_ option: String) -> ChartEasingOption {
        let firstStr = option.substring(to: option.index(option.startIndex, offsetBy: 1));
        let start1 = option.index(option.startIndex, offsetBy: 1)
        let nextStr = option.substring(from: start1)
        let iosEnumString = firstStr.lowercased() + nextStr
